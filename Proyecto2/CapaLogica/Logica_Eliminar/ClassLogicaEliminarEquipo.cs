...
{
    public class ClassLogicaEliminarEquipo
    {
        public void EliminarEquipo(string ID, Page pagina)
        {
            Class_Equipos_data.EquipoID = Convert.ToInt32(ID);
          
            string connectionString = ConfigurationManager.ConnectionStrings["SQLconnection"].ConnectionString;

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("sp_EliminarEquipo", connection))
                {
                    connection.Open();
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;


                    cmd.Parameters.Add(new SqlParameter("@EquipoID", Class_Equipos_data.EquipoID));
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        MostrarMensaje(pagina, "Equipo eliminado exitosamente.");
                    }
                    else
                    {
