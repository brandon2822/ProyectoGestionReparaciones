...
{
    public class ClassLogicaEliminarUsuario
    {
       public void EliminarUsuario(string ID,Page pagina)
        {
            Class_Usuarios_data.UsuarioID = Convert.ToInt32(ID);
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("sp_EliminarUsuario", connection))
                {
                    ...
                    cmd.Parameters.Add(new SqlParameter("@UsuarioID", Class_Usuarios_data.UsuarioID));
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        MostrarMensaje(pagina,"Usuario eliminado exitosamente.");
                    }
                    else
                    {
