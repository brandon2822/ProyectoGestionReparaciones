...
{
    public class ClassLogicaEliminarReparacion
    {
        public void EliminarReparacion(string ID, Page pagina)
        {
            Class_Reparaciones_data.ReparacionID = Convert.ToInt32(ID);

            string connectionString = ConfigurationManager.ConnectionStrings["SQLconnection"].ConnectionString;

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("sp_EliminarReparacion", connection))
                {
                    connection.Open();
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;


                    cmd.Parameters.Add(new SqlParameter("@ReparacionID", Class_Reparaciones_data.ReparacionID));
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        MostrarMensaje(pagina, "Reparacion eliminada exitosamente.");
                    }
                    else
                    {
